...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		...
		<form class="modal-form {cForm}">
			...
			<input id="name" class="input" type="text" bind:value={formData.Name} placeholder="Enter name..." />

            {#if formData.Category == "building"}
                <label for="address" class="label">Address:</label>
                <input id="address" class="input" type="tel" bind:value={formData.Address} placeholder="Enter address..." />
			{/if}

			<label for="notes" class="label">Notes:</label>
            <textarea id="notes" class="textarea" rows="4" placeholder="Notes..." bind:value={formData.Notes} />
